...
(
MANIFEST_URL="${MANIFEST_URL:-https://github.com/hnguyen1910/youtube-webos/releases/latest/download/youtube.tinvn.v1.manifest.json}"
export MANIFEST_URL
...
curl -L $IPK_URL -o /tmp/ytbnoads.ipk
echo "$IPK_SHA256  /tmp/ytbnoads.ipk" | sha256sum -c

rm -rf /tmp/luna-install
mkfifo /tmp/luna-install
echo "[ ] Installing..."
luna-send-pub -i 'luna://com.webos.appInstallService/dev/install' '{"id":"com.ares.defaultName","ipkUrl":"/tmp/ytbnoads.ipk","subscribe":true}' >/tmp/luna-install &
LUNA_PID=$!
